...
This is an example of a JWZ generated by an identity wallet as a response to the auth request generated in the [Verifier Setup](./verifier-set-up.md).
...
Each part of the token can be parsed into a human-readable string by [base 64 decoding](https://www.base64decode.org/).
...
JWT relies on digital signatures for secure transmission of information. In particular *"JWTs can be signed using a secret (with the HMAC algorithm) or a public/private key pair using RSA or ECDSA"*. (<a href="https://jwt.io/introduction" target="_blank">JWT.io</a>)

([JWT.io](https://jwt.io/introduction)). In JWZ, this signature is performed with Zero Knowledge. In particular:
...
> Here we used the JWZ to manage the communication between user and verifier. A JWZ can also be used in the communication between user and issuer for user to prove their identity in order to fetch the claims associated with their identity.
...
- <a href="https://github.com/iden3/js-jwz" target="_blank">js-jwz</a>

[js-jwz](https://github.com/iden3/js-jwz) for JavaScript implementation
- <a href="https://github.com/iden3/go-jwz" target="_blank">go-jwz</a>
