...
As long as the user has been issued a claim that contains certain information, the Verifier can design a specific query related to the claim based on these 5 operators. 
...
> To add an operator-based query inside a proof, request is not mandatory. For instance, a platform can just ask the user to have an AgeCredential claim as a way to identify unique humans without any requirements related to the date of birth. In that case, the Veriifer should not simply fill the `req` field inside the proofRequest.
