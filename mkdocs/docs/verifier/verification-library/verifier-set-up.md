...
4. **Callback Endpoint**
	...
	!!!info "Testnet or Mainnet"
		The code samples on this page are using Polygon's Testnet Mumbai, including the smart contract address and the RPC endpoint in the `ethURL` variable. If you want to use the Mainnet, you need to:
		
		1. Change the identity state `contractAddress` from `0x134B1BE34911E39A8397ec6289782989729807a4`(Mumbai) to `0x624ce98D2d27b20b8f8d521723Df8fC4db71D79D`(Mainnet).
		2. Change DID prefix from `polygon:mumbai` to `polygon:main`, where applicable.
		3. Use the **Polygon main URL** from Alchemy, Infura or any other infrastructure provider. 
