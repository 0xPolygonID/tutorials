...
The prerequisite is that users have the [Polygon ID Wallet app](../../wallet/wallet-overview.md) installed and self-issued a Credential of type `KYC Age Credential Merklized` using our [Issuer Sandbox](https://issuer-v2.polygonid.me/). Further credentials can be issued using the [Issuer Node](../../issuer/issuer-overview.md).
