...
If no previously created Identifier is found, the SDK first needs to create an identity first using `identity.addIdentity()` function. 
...
```dart
PolygonIdSdk.I.identity.getDidIdentifier(
