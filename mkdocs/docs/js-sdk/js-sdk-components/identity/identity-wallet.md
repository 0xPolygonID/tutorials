...
- Key Management Store (kms): it allows the user to sign data using keys and is the storage that is allocated to these keys.  It’s an abstraction that allows to communicate with different kinds of key storage.
...
This method creates an Auth Baby Jub Jub Credential (for signing on behalf of an identity) and an Identifier for an Identity. The Baby Jub Jub Key is the elliptic curve implementation used in Iden3.
...
!!! info
    To know more on Iden3 core elements (Baby Jub Jub Key, Credentials, and Identifiers, read Iden3 tutorials [here](https://docs.iden3.io/getting-started/getting-started/).
...
!!! info
    To know more about the credential's inclusion in a Merkle tree, read our Iden3 core specification tutorials [here](https://docs.iden3.io/protocol/spec/#update-claims).
